...
public class PreferencesTools {
     ...
     * 保存T数据
     */
    public static <T> boolean saveObj(Context context, String savingName, T entity, boolean isEcrypt) {
        try {
            SharedPreferences settings = context.getSharedPreferences(savingName, Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = settings.edit();
            Gson gson = new Gson();
     ...
     * @return
     */
    public static <T> T  getObjList(Context context, String savingName ,Class<T> classType, boolean isEcrypt) {
        T list = null;
        SharedPreferences settings;
        try {
             settings = context.getSharedPreferences(savingName, Context.MODE_PRIVATE);
            String data = settings.getString(classType.getName(), null);
            if (!TextUtils.isEmpty(data)) {
                String name = new DeviceUuidFactory(context).getDeviceUuid().toString();//绑定设备信息，动态生成密码
                if (isEcrypt) {
                    String messageAfterDecrypt = AESCrypt.decrypt(name, data);
                    list = new Gson().fromJson(messageAfterDecrypt, (Type) classType);
                } else {
                    list = new Gson().fromJson(data, (Type)classType);
                }
            }
        } catch (Exception error) {
     ...
     * @return
     */
    public static <T> T getObj(Context context, String savingName, Class<T> classType, boolean isEcrypt) {
        T user = null;
        try {
            SharedPreferences settings = context.getSharedPreferences(savingName, Context.MODE_PRIVATE);
            String data = settings.getString(classType.getName(), null);
            ...
            if (!TextUtils.isEmpty(data)) {
                ...
                if (isEcrypt) {
                    ...
                    Gson gson = new Gson();
                    user = gson.fromJson(data, classType);
                }
            }
        } catch (Exception error) {
            error.printStackTrace();
